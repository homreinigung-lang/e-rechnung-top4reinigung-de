ALTER TABLE public.documents ADD COLUMN IF NOT EXISTS storno_reason text NOT NULL DEFAULT '';
...
AS $function$
DECLARE src public.documents; new_id uuid; n text; uid uuid := auth.uid(); r text := btrim(coalesce(_reason, ''));
BEGIN
  IF uid IS NULL THEN RAISE EXCEPTION 'Nicht angemeldet'; END IF;
  IF length(r) < 3 THEN RAISE EXCEPTION 'Bitte geben Sie einen Stornogrund an.'; END IF;
  ...
  VALUES (src.user_id, new_id, n, 'storno_created', jsonb_build_object('cancels', src.number, 'total', -src.total, 'reason', r)),
         (src.user_id, src.id, src.number, 'cancelled', jsonb_build_object('storno_number', n, 'reason', r));
