...
Record the exact Git SHA externally when the package is frozen for the real backup/rehearsal. Do not edit this file merely to embed its own resulting commit SHA.

- Git branch: `migration/final-cleanup`
- Release commit: record in `release/release-sha.txt`
