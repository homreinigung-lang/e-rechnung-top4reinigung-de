-- Internal helper: must not be callable directly by app users
...
REVOKE ALL ON FUNCTION public.document_items_enforce_immutability() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.documents_enforce_immutability() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.update_updated_at_column() FROM PUBLIC, anon, authenticated;

-- User-facing RPCs: keep for authenticated only, block anon
...
-- Explicit caller checks inside the SECURITY DEFINER RPCs
...
AS $function$
DECLARE d public.documents; n text; uid uuid := auth.uid();
BEGIN
